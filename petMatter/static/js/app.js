...
function agregarItemAlCarrito(titulo, precio, imagenSrc,cantidad=1) {
  ...
  for (var i = 0; i < nombresItemsCarrito.length; i++) {
    if (nombresItemsCarrito[i].innerText == titulo) {
     ...
     let precioTotal = document.querySelector(".carrito-precio-total").textContent
      console.log(precioTotal);
     let precioItem = parseFloat(precio.replace("$", "").replace(".", ""));
     let precioTotalNumber = parseFloat(precioTotal.replace("$", "").replace(".", ""));
     document.querySelector(".carrito-precio-total").textContent = `$${(precioItem+precioTotalNumber).toLocaleString("es")},00` 
     return;
    }
  }
...
function sumarCantidad(event) {
  ...
  const nombreItem = selector.parentElement.querySelector(".carrito-item-titulo").textContent
...
function eliminarItemCarrito(event) {
  ...
  const nombreItem = buttonClicked.parentElement.parentElement.querySelector(".carrito-item-titulo").textContent;
  const storage = JSON.parse(localStorage.getItem("myStorage"));
